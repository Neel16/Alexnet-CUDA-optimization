...
void NeuralNetwork()
{
	...
	printf("\n Elapsed time for layer 1 is %.2f ms", milliseconds);
	milliseconds = 0;
	...
	cudaMalloc((void**) &Layer2_bias_GPU, sizeof(float)* L2_OUT); //made consatant
	...
	//cudaMemcpyToSymbol(bias_L2_CM,bias_2, sizeof(float)* L2_OUT);////made constant
	...
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 2 is %.2f ms", milliseconds);
	...
	milliseconds = 0;	
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 3 is %.2f ms", milliseconds);
	...
	milliseconds = 0;	
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 4 is %.2f ms", milliseconds);
	...
	milliseconds = 0;	
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 5 is %.2f ms", milliseconds);
	...
	milliseconds = 0;	
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 6 is %.2f ms", milliseconds);
	// RELU Layer 
	...
	//cudaMalloc((void**) &Layer7_bias_GPU, sizeof(float)*4096);//making this as constant memory
	...
	////cudaMemcpy(Layer7_bias_GPU,bias_7, sizeof(float)*4096,cudaMemcpyHostToDevice);//making this as constant memory
	cudaMemcpyToSymbol(bias_L7_CM,bias_7, sizeof(float)*4096);
	cudaMemcpyToSymbol(Layer_InNeurons_GPU_L7_CM,Layer7_Neurons_GPU,sizeof(float)*4096);////making this as constant memory

	dim3 Layer7_Block(4096,1,1);
	dim3 Layer7_Thread(1,1);   // combi tried 10*10*10
	cudaEventRecord(start);
	executeFCLayer_L7<<<Layer7_Block,Layer7_Thread>>>(Layer7_Weights_GPU,Layer8_Neurons_GPU,4096,4096,true,false);
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
	milliseconds = 0;	
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 7 is %.2f ms", milliseconds);
	...
	//cudaMalloc((void**) &Layer8_bias_GPU, sizeof(float)*1000);//making this as constant memory
	...
	//cudaMemcpy(Layer8_bias_GPU,bias_8, sizeof(float)*1000,cudaMemcpyHostToDevice);//making this as constant memory
	cudaMemcpyToSymbol(bias_L8_CM,bias_8, sizeof(float)*1000);
	//cudaMemcpyToSymbol(Layer_InNeurons_GPU_L8_CM,Layer8_Neurons_GPU,sizeof(float)*4096);////OVERFLOW ACCUMULATION OF CM

	dim3 Layer8_Block(1000,1,1);
	dim3 Layer8_Thread(1,1);   // combi tried 10*10*10
	cudaEventRecord(start);
	executeFCLayer_L8<<<Layer8_Block,Layer8_Thread>>>(Layer8_Neurons_GPU,Layer8_Weights_GPU,Layer9_Neurons_GPU,1000,4096,false,false);
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
	milliseconds = 0;	
	cudaEventElapsedTime(&milliseconds, start, stop);
	printf("\n Elapsed time for layer 8 is %.2f ms", milliseconds);
