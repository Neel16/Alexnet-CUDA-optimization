...
__global__ void executeFirstLayer(float *bias,float *Layer1_Neurons_GPU,float *Layer1_Weights_GPU,float *Layer2_Neurons_GPU,int r_offset, int c_offset)
{
	...
	#pragma unroll 
    for(int i = 0; i < 11; i++) //loop unrolling 
    {
        for(int j = 0; j < 11; j++)
        {
...
////copying same function for Layer 6
__constant__ float bias_L6_CM[4096];
__global__ void executeFCLayer_L6(float *Layer_InNeurons_GPU, float *Layer_Weights_GPU,float *Layer_OutNeurons_GPU,int output, int input,bool reLU,bool dropout)
{

	////__shared__ float Layer_InNeurons_GPU_SM[256*6*6];////no point in making this as shared memory or constant as it is huge >10KB
...
__global__ void executeFCLayer_L7(float *Layer_Weights_GPU,float *Layer_OutNeurons_GPU,int output, int input,bool reLU,bool dropout)
{
...
////copying same function for Layer 8
__constant__ float bias_L8_CM[1000];
//__constant__ float Layer_InNeurons_GPU_L8_CM[4096];
__global__ void executeFCLayer_L8(float* Layer_InNeurons_GPU_L8,float *Layer_Weights_GPU,float *Layer_OutNeurons_GPU,int output, int input,bool reLU,bool dropout)
{
